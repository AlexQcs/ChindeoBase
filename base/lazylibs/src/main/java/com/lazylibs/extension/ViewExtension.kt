 ...
 * @return T
 */
fun <T : View> T.withTrigger(delay: Long = 200): T {
    triggerDelay = delay
    return this
}
...
fun <T : View> T.longClickWithTrigger(time: Long = 800, block: (T) -> Unit) =
 ...
 * @return Unit
 */
fun <T : View> T.clickWithTrigger(time: Long = 200, block: (T) -> Unit) {
    triggerDelay = time
    setOnClickListener {
        if (clickEnable()) {
            block(it as T)
...
fun <T : EditText> T.setText(text: String?) {
    this.setText(text)
}

fun <T : EditText> T.setText2(strText: String?) {
    this.text = Editable.Factory.getInstance().newEditable(strText)
}
